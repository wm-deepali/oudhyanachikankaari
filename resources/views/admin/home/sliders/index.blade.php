...
<div class="main-section">
    ...
    .slider-img { width: 90px; height: 56px; object-fit: cover; border-radius: var(--radius-sm); border: 1px solid var(--border); display: block; }
    .slider-img-placeholder { width: 90px; height: 56px; border-radius: var(--radius-sm); background: var(--bg); border: 1px solid var(--border); display: flex; align-items: center; justify-content: center; color: var(--text-hint); font-size: 18px; }
    ...
    .link-cell { font-size: 12px; color: var(--accent); font-family: 'SF Mono','Fira Code',monospace; max-width: 200px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; display: block; }
    ...
    <div class="app-content content container-fluid">
        <div class="list-page">
            ...
            <div class="list-page-header">
                ...
                <a href="{{ route('admin.home.sliders.create') }}" class="btn-primary-dash">
                        ...
                        <tbody>
                            @forelse($sliders as $slider)
                                <tr id="row{{ $slider->id }}">
                                    ...
                                    <td><span class="id-chip">{{ $slider->id }}</span></td>

                                    <td>
                                        @if($slider->image)
                                            <img src="{{ asset('storage/'.$slider->image) }}"
                                                class="slider-img" alt="Slider #{{ $slider->id }}">
                                        @else
                                    ...
                                    <td>
                                        @if($slider->link)
                                            <a href="{{ $slider->link }}" target="_blank" class="link-cell" title="{{ $slider->link }}">
                                                {{ $slider->link }}
                                            </a>
                                        @else
                                            <span style="color:var(--text-hint);font-size:12px">No link</span>
                                        @endif
                                    </td>

                                    <td><span class="sort-chip">{{ $slider->sort_order }}</span></td>
                                ...
                                <tr>
                                    <td colspan="6">
                                        <div class="empty-state">
                                            ...
                                            <p>Add homepage slider images to display a banner carousel.</p>
                                            <a href="{{ route('admin.home.sliders.create') }}" class="btn-primary-dash">
                                                <i class="fa fa-plus"></i> Add Slider
                                            </a>
                                        </div>
